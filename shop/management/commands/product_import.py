...
def def_category(category_id, root_xml):
    cat_name = root_xml.find(f".//category[@id='{category_id}']")  # Нахожу катеорию через id в загруженном xml
    cat = cat_name.text
    parent = Category.objects.get(name=root_xml.find(f".//category[@id='{cat_name.attrib['parentId']}']").text)
    category = Category.objects.get(name=cat, parent=parent)
    return category.id
